...
export async function POST(
  ...
  if (input.action === "schedule") {
    ...
    const { data: insertedJob, error: jobError } = await supabase
      ...
      .select(
        "id, lead_id, estimate_id, parent_job_id, title, status, scheduled_start, scheduled_end, service_name, customer_name, location_address, location_lat, location_lng, notes, recurrence_rule, units_completed, rate_per_unit, total_revenue, created_by, created_at, gcal_event_id, gcal_synced_at"
      )
      .single();
